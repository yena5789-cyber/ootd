...
type ModalState = { type: "request"; candidate: Candidate } | { type: "register" } | null;
...
export default function EnaVerseApp() {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md overflow-hidden bg-cream pb-28 text-cocoa">
      <section className="relative px-5 pb-8 pt-8">
        <div className="absolute -right-20 -top-20 h-56 w-56 rounded-full bg-peach/25 blur-3xl" />
        <div className="absolute -left-24 top-44 h-52 w-52 rounded-full bg-amber-200/50 blur-3xl" />
        <div className="relative rounded-[2rem] border border-white/80 bg-white/85 p-6 shadow-card backdrop-blur">
          <p className="mb-3 inline-flex rounded-full bg-blush px-3 py-1 text-xs font-bold text-peach">
            지인이니까 믿고, 지인이니까 편하게
          </p>
          <h1 className="text-4xl font-black tracking-tight">에나버스</h1>
          <p className="mt-3 text-lg font-bold leading-7">에나의 지인망 안에서 시작되는 비공개 소개</p>
          <p className="mt-4 text-sm leading-6 text-stone-600">
            모든 후보는 실제 지인 연결 경로를 확인한 뒤 등록됩니다. 사진과 연락처는 양측 동의 후에만 전달됩니다.
          </p>
          <div className="mt-6 grid gap-3">
            <a href="#candidates" className="rounded-2xl bg-cocoa px-5 py-4 text-center text-sm font-black text-white shadow-soft">
              소개 후보 보기
            </a>
            <button
              type="button"
              onClick={() => setModal({ type: "register" })}
              className="rounded-2xl border border-rose-100 bg-blush px-5 py-4 text-sm font-black text-peach"
            >
              내 프로필 등록하기
            </button>
          </div>
      ...
      <section id="candidates" className="px-5">
        <div className="mb-4 rounded-3xl border border-amber-100 bg-amber-50/80 p-4 text-xs leading-5 text-stone-600">
          <p className="font-black text-cocoa">지인 연결 기준</p>
          <p className="mt-1">1촌은 에나 직접 지인, 2촌은 에나 친구의 친구, 3촌은 지인 경로 확인 완료를 의미합니다.</p>
        </div>

        <div className="sticky top-0 z-10 -mx-5 bg-cream/90 px-5 py-3 backdrop-blur">
            ...
            {filters.map((filter) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2 text-sm font-black transition ${
                  activeFilter === filter ? "bg-cocoa text-white shadow-md" : "border border-stone-200 bg-white text-stone-600"
                }`}
              >
                {filter}
              </button>
      ...
      <section className="mx-5 mt-8 rounded-3xl border border-stone-200 bg-white p-5 text-xs leading-6 text-stone-600 shadow-card">
        <p className="mb-2 text-sm font-black text-cocoa">프라이버시 안내</p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-md border-t border-stone-200 bg-white/95 p-4 backdrop-blur">
        <button
          type="button"
          onClick={() => setModal({ type: "register" })}
          className="w-full rounded-2xl bg-peach px-5 py-4 text-sm font-black text-white shadow-soft"
        >
          카카오톡으로 후보 등록하기
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-30 flex items-end justify-center bg-stone-950/45 px-4" role="dialog" aria-modal="true">
          <div className="mb-4 max-h-[86vh] w-full max-w-md overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black text-peach">에나버스 카카오톡 연결</p>
                <h2 className="mt-1 text-xl font-black text-cocoa">
            ...
            <div className="mt-4 grid gap-3">
              <button
                ...
                className="rounded-2xl bg-cocoa px-5 py-4 text-sm font-black text-white"
              >
...
function getAgeRange(birthYear: number) {
  ...
  return (
    <article className="rounded-[1.75rem] border border-white bg-white p-5 shadow-card">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-black text-peach">[{candidate.id}]</p>
          <h2 className="mt-1 text-lg font-black text-cocoa">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-stone-700">
        ...
        <Info label="지인 연결" value={`${candidate.connectionDegree} (${candidate.connectionLabel})`} />
      </div>

      <CardSection title="성격" text={candidate.personality} />
      <CardSection title="취미" text={candidate.hobbies} />
      ...
      <button type="button" onClick={onRequest} className="mt-5 w-full rounded-2xl bg-peach px-5 py-4 text-sm font-black text-white shadow-soft">
        소개 요청하기
      </button>
    </article>
  );
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <p className="rounded-2xl bg-stone-50 px-3 py-2">
      <span className="font-black text-cocoa">{label}: </span>
      {value}
    </p>
  );
}
